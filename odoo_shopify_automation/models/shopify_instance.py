...
class ShopifyInstance(models.Model):
  ...
  shop_url = fields.Char('Shopify Shop URL',
                         required=True,
                         help='e.g. https://yourstore.myshopify.com')
  ...
  _sql_constraints = [
      ('shop_url_uniq', 'unique(shop_url, company_id)',
  ...
  def _cron_sync_orders(self):
    """Cron method for syncing orders"""
    self.env['shopify.order']._run_order_import_cron()

  @api.model
  def _cron_sync_customers(self):
    """Cron method for syncing customers"""
    self.env['shopify.customer']._run_customer_import_cron()

  @api.model
  def _cron_sync_inventory(self):
    """Cron method for syncing inventory"""
    # Implementation would go here for inventory sync
    pass

      ...
      if response.status_code == 200:
        ...
        self.env['shopify.log'].create({
            ...
            'job_id':
                job.id,
            'message':
                f'Starting import of {len(customers)} customers from Shopify instance {instance.name}',
        })
          ...
          try:
            ...
            self.env['shopify.log'].create({
                ...
                'job_id':
                    job.id,
                'message':
                    f'Error importing customer {shopify_customer.get("email", "Unknown")}: {str(e)}',
            })
    ...
    for instance in instances:
      try:
        ...
        self.env['shopify.log'].create({
            ...
            'log_type': 'error',
            'message': f'Error importing customers for instance {instance.name}: {str(err)}',
        })

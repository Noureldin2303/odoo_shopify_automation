    ...
    for cron in self:
      try:
        if cron.cron_type == 'import_product':
          ...
          # Use the dedicated cron method that finds pending orders correctly
          self.env['shopify.order']._run_order_export_cron()
        ...
        self.env['shopify.log'].create({
            'name':
                f'Cron {cron.cron_type} error',
            'log_type':
                'error',
            'message':
                f'Error in cron job {cron.cron_type} for instance {cron.instance_id.name}: {str(e)}',
        })

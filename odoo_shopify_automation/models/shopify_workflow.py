...
class ShopifyWorkflow(models.Model):
  ...
  instance_id = fields.Many2one('shopify.instance',
  ...
  average_execution_time = fields.Float('Average Execution Time (seconds)',
                                        default=0.0,
                                        tracking=True)
  ...
  def action_test_workflow(self):
    ...
    for record in self:
      try:
        ...
        record.message_post(body=_('Workflow test failed: %s') % str(err))
        raise UserError(_('Workflow test failed: %s') % str(err))
  ...
  def action_execute_workflow(self, order):
    ...
    for record in self:
      ...
      try:
        ...
        record.message_post(body=_('Workflow execution failed for order %s: %s') % (order.name, str(err)))
        raise UserError(_('Workflow execution failed: %s') % str(err))
  ...
  @api.model
  def _cron_process_workflows(self):
    try:
      # Process active workflows that need attention
      active_workflows = self.search([('state', '=', 'active'), ('auto_process', '=', True)])

      processed_count = 0
      for workflow in active_workflows:
        try:
          # Check for orders that match this workflow
          if workflow.workflow_type == 'order_processing':
            orders = self.env['shopify.order'].search([
                ('instance_id', '=', workflow.instance_id.id),
                ('shopify_order_status', '=', workflow.shopify_status),
                ('state', '!=', workflow.odoo_status)
            ])
      ...
      _logger.error(f"Error in workflow cron job: {str(err)}")

  @api.model
  def process_order_status_change(self, order, new_status):
    """Process order status change through workflows"""
    workflows = self.search([('instance_id', '=', order.shopify_instance_id.id),
                             ('shopify_status', '=', new_status),
                             ('workflow_type', '=', 'order_processing'), ('state', '=', 'active')])
...
class ShopifyWorkflowStep(models.Model):
  ...
  workflow_id = fields.Many2one('shopify.workflow',
                                string='Workflow',
                                required=True,
                                ondelete='cascade')
  ...
  def _execute_step(self, order):
    ...
    try:
      ...
      self.error_count += 1
      _logger.error('Step execution failed: %s', str(err))
  ...
  def _check_condition(self, order):
    ...
    try:
      ...
      _logger.error('Error checking condition: %s', str(err))
      return False

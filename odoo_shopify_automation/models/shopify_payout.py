...
class ShopifyPayout(models.Model):
  ...
  # Reconciliation
  reconciled_amount = fields.Monetary('Reconciled Amount',
  ...
  exchange_rate = fields.Float(
      'Exchange Rate',
      default=1.0,
      help='Exchange rate for currency conversion',
  )
  base_currency_amount = fields.Monetary('Base Currency Amount',
  ...
  def _compute_analytics(self):
    for record in self:
      ...
      record.chargeback_count = len(
          transactions.filtered(lambda t: t.transaction_type == 'chargeback'))
  ...
  def _compute_reconciliation(self):
    for record in self:
      ...
      record.reconciliation_rate = (record.reconciled_amount / record.amount *
                                    100) if record.amount else 0
    ...
    for record in self:
      if record.currency_id and record.base_currency_id:
        record.base_currency_amount = record.currency_id._convert(
            record.amount, record.base_currency_id, record.env.company, record.payout_date or
            fields.Date.today())
      else:
      ...
      if transaction.transaction_type in ['sale', 'refund']:
        statement_line_vals = {
            'statement_id':
                statement.id,
            'date':
                transaction.processed_at or self.payout_date,
            'name':
                f"{transaction.transaction_type.title()} - {transaction.source_order_id or transaction.transaction_id}",
            'amount':
                transaction.net_amount,
            'partner_id':
                self._get_partner_from_order(transaction.source_order_id),
        ...
        if sale_order:
          ...
          if invoice:
            ...
            statement_line = self.env['account.bank.statement.line'].search(
                [('shopify_transaction_id', '=', transaction.id)], limit=1)
  ...
  def _get_partner_from_order(self, order_id):
    ...
    if order_id:
      sale_order = self.env['sale.order'].search(
          [('shopify_order_id', '=', order_id), ('shopify_instance_id', '=', self.instance_id.id)],
          limit=1)
  ...
  def import_payouts_from_shopify(self, instance, start_date=None, end_date=None):
    ...
    for payout_data in payouts_data:
      ...
      existing_payout = self.search([('payout_reference_id', '=', payout_id),
  ...
  def get_payout_summary(self):
    ...
    return {
        ...
        'total_fees':
            sum(self.mapped('fee_amount')),
        'total_net':
            sum(self.mapped('net_amount')),
        'reconciled_amount':
            sum(self.mapped('reconciled_amount')),
        'unreconciled_amount':
            sum(self.mapped('unreconciled_amount')),
        'avg_reconciliation_rate':
            sum(self.mapped('reconciliation_rate')) / len(self) if self else 0,
    }
